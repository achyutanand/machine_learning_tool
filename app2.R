  ...
  mainPanel(
    ...
    tabsetPanel(
      ...
      tabPanel("Segmentation", uiOutput("fileEntry"),
               ...
               bsModal("dend_plot", title = "Dendogram of clusters", trigger = "Plot_dend",
                       plotOutput("dend")),
               #         plotOutput("dend"),
               uiOutput("treecut"),
               uiOutput("op_tp_hc"),
  ...
  output$summary <- renderDataTable(if(input$Go3){
    ...
    mystats <- function(x){
      if(class(x)=="numeric"){
        ...
        p2 <- quantile(x, 0.05, na.rm = T)
        p3 <- quantile(x, 0.10, na.rm = T)
        ...
        p50 <- quantile(x,0.50, na.rm = T)
        p75 <- quantile(x,0.75, na.rm = T)
        p90 <- quantile(x,0.90, na.rm = T)
        p95 <- quantile(x,0.95, na.rm = T)
        p99 <- quantile(x,0.99, na.rm = T)
        skw <- skewness(x, na.rm = T, 3)
        y <- na.omit(x)
        lm <- length(unique(y))
        maxi <- max(x, na.rm = T)
  ...
  output$model <- renderPrint(if(input$Go){
    ...
    if(isolate(input$tuning) == 1){
      ...
      if(isolate(input$MLT)==1){
        ...
        fit.rf <<- train(Formula, data = training_set, method = "rf", metric = Metric,
                         preProcess = c("center","scale"), importance = T, trControl = tc11, tuneGrid = rfgrid)
        testing_set$pred <- predict(fit.rf, testing_set)
        results <- resamples(list(RandomForest=fit.rf, RandomForest=fit.rf))
        #       fit.rf       
